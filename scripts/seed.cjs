...
const contentBlocks = [
  ...
  ["catalog", "Заголовок каталога", "Каталог для запроса КП", "Выберите позиции — они сохранятся в форме заявки даже после перезагрузки страницы. Розничные цены не выводятся: стоимость фиксируется в коммерческом предложении.", "", "", "", "", 40],
  ...
  ["request", "Форма заявки", "Соберём запрос по вашему объекту", "Проверьте состав КП, заполните контакты — заявка сохранится в панели управления и придёт менеджеру в Telegram/на почту, если уведомления настроены.", "", "", "", "", 90],
];
...
async function main() {
  ...
  try {
    ...
    for (const [index, title] of uniqueSeries.entries()) {
      ...
      // Галерея: обновляем только если пуста; описание — только если ещё дефолтный текст.
      await pool.query(
        "UPDATE series SET gallery = $1 WHERE slug = $2 AND gallery = '[]'",
        [galleryJson, slug],
      );
      await pool.query(
        "UPDATE series SET short_description = $1 WHERE slug = $2 AND short_description = 'Серийная модель для комплектации объектов.'",
